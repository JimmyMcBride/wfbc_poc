<script lang="ts">
	...
	let expandedStaff = $state<number | null>(null);
	...
	function toggleStaff(i: number) {
		expandedStaff = expandedStaff === i ? null : i;
	}
</script>
...
<section class="py-20 bg-(--color-bg-alt)">
	<div class="container text-center">
		...
		<h2 class="text-[clamp(1.75rem,4vw,2.5rem)] mb-10">Pastors &amp; Ministry Staff</h2>
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
			{#each staff as member, i}
				<button class="staff-card flex flex-col items-center text-center p-6 bg-(--color-bg-card) border border-(--color-border-light) rounded-[var(--radius-lg)] cursor-pointer transition-all duration-300 w-full hover:border-(--color-border) hover:shadow-(--shadow-sm)" class:expanded={expandedStaff === i} onclick={() => toggleStaff(i)}>
					...
					<h3 class="font-sans text-base font-semibold text-(--color-heading) mb-1">{member.name}</h3>
					...
					{#if member.bio}
						<span class="inline-flex items-center gap-1 mt-3 text-[0.8rem] font-semibold text-(--color-accent)">
							{expandedStaff === i ? 'Less' : 'Read more'}
							<svg class:rotated={expandedStaff === i} xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="transition-transform duration-200"><polyline points="6 9 12 15 18 9"></polyline></svg>
						</span>
					{/if}
				</button>
			{/each}
	...
	<div class="container max-w-[900px] text-center">
		...
		<div class="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-10">
			<div class="bg-(--color-bg-card) border border-(--color-border-light) rounded-[var(--radius-lg)] p-8 px-6 transition-all duration-300 hover:border-(--color-primary) hover:shadow-(--shadow-sm)">
				<div class="flex items-center justify-center w-13 h-13 mx-auto mb-4 bg-(--color-primary-light) text-(--color-primary) rounded-full">
